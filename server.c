...
void serverChat(int fd)
{
	...
	while(1)
	{
		...
		//print buffer
		printf("Client says: %sServer says:",buf);
		bzero(buf,BUF_SIZE);
...
int main()
{
	 ...
	 */
	if(soc==-1){
		printf("Socket creation at server failed with :  %d!\n",soc);
		exit(0);
	}
	else{

		printf("Socket creation at server successful!\n");
	}
	...
	if(b!=0){
		printf("Socket binding failed with errcode: %d\n",b);
		exit(0);
	}
	else{
	...
	if(fd<0){
		printf("Data packet from client not accepted!!! %d\n",fd);
		exit(0);
	}
	else{
